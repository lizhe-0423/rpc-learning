...
public class RpcProviderHandler extends SimpleChannelInboundHandler<RpcProtocol<RpcRequest>> {
     ...
     */
    @Override
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, RpcProtocol<RpcRequest> protocol) {
        // 记录接收到的数据内容
        LOGGER.info("RPC提供者收到的数据为==== >>> {}", JSON.toJSONString(protocol));
        ...
        RpcProtocol<RpcResponse> responseRpcProtocol = getRpcResponseRpcProtocol(protocol);
        // 返回接收并处理后的数据
